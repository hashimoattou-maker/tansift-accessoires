          ...
          <form id="articleForm" onsubmit="saveArticle(event, ${articleId})">
            ...
            <div class="form-row">
              ...
              <div class="form-group"><label>Type</label><select name="type_article" class="form-select">
                ${['accessoire','moteur','assemblage','capo','masque','parechoc','ailes','portes','mala','plafond','dynamo','demareurs','radiateur','parabole','feu_rouge'].map(t => `<option value="${t}" ${a.type_article===t?'selected':''}>${t.charAt(0).toUpperCase()+t.slice(1).replace('_',' ')}</option>`).join('')}
              </select></div>
            </div>
            <div class="form-row">
      ...
      openModal(title, html`
        <form id="articleForm" onsubmit="saveArticle(event)">
          ...
          <div class="form-row">
            ...
            <div class="form-group"><label>Type</label><select name="type_article" class="form-select"><option value="accessoire">Accessoire</option><option value="moteur">Moteur</option><option value="assemblage">Assemblage</option><option value="capo">Capo</option><option value="masque">Masque</option><option value="parechoc">Parechoc</option><option value="ailes">Ailes</option><option value="portes">Portes</option><option value="mala">Mala</option><option value="plafond">Plafond</option><option value="dynamo">Dynamo</option><option value="demareurs">Demareurs</option><option value="radiateur">Radiateur</option><option value="parabole">Parabole</option><option value="feu_rouge">Feu rouge</option></select></div>
          </div>
          <div class="form-row">
...
function editDocument(id) {
  apiFetch(`/documents/${id}`).then(d => {
    ...
    let htmlContent = html`
      ...
      <div class="stat-item"><div class="stat-value">${canEdit ? `<input type="date" id="editDocDate" class="form-control" value="${(d.date_document||'').slice(0,10)}" style="width:130px;font-size:0.85rem;text-align:center;padding:0.2rem 0.4rem;border:none;background:transparent;font-weight:600;color:var(--text)">` : formatDate(d.date_document)}</div><div class="stat-label">Date</div></div>
      ...
      ${canEdit ? `<div class="stat-item"><div class="stat-value"><input type="text" id="editDocRefExterne" class="form-control" value="${d.ref_externe || ''}" placeholder="N° externe..." style="width:130px;font-size:0.85rem;text-align:center;padding:0.2rem 0.4rem;border:none;background:transparent;font-weight:600;color:var(--text)"></div><div class="stat-label">N° externe</div></div>` : (d.ref_externe ? `<div class="stat-item"><div class="stat-value">${d.ref_externe}</div><div class="stat-label">N° externe</div></div>` : '')}</div>

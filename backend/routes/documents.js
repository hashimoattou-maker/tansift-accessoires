...
module.exports = function(db) {
  ...
  router.post('/', (req, res) => {
    ...
    const numero = generateDocumentNumber(db, type_document);
    const result = db.prepare(`INSERT INTO documents (type_document, numero, client_id, fournisseur_id, utilisateur_id, notes, conditions_paiement, adresse_livraison) VALUES (?,?,?,?,?,?,?,?)`)
    ...
    if (lignes && Array.isArray(lignes)) {
      const insertLigne = db.prepare(`INSERT INTO documents_lignes (document_id, article_id, ligne_numero, reference, designation, quantite, prix_unitaire_ht, remise_pourcent, taux_tva, montant_ht, montant_tva, montant_ttc, marge_brute) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`);
      let totalHT = 0, totalTVA = 0, totalTTC = 0;

      const insertBatch = db.transaction((lignes) => {
        for (const ligne of lignes) {
          ...
          if (ligne.article_id) {
            const article = db.prepare(`SELECT * FROM articles WHERE id = ?`).get(ligne.article_id);
            if (article) {
          ...
          insertLigne.run(docId, ligne.article_id || null, ligne.ligne_numero || 0, ligne.reference || null, ligne.designation || null, qte, prix, remise, tva, montantHT, montantTVA, montantTTC, marge);
        }
      });
      insertBatch(lignes);

      // Update document totals
      db.prepare(`UPDATE documents SET montant_ht = ?, total_tva = ?, montant_ttc = ?, net_a_payer = ? WHERE id = ?`)
      ...
      if (['bon_livraison', 'facture_client', 'bon_reception', 'facture_fournisseur'].includes(type_document)) {
        const movBatch = db.transaction(() => {
          for (const ligne of lignes) {
            if (!ligne.article_id) continue;
            const article = db.prepare(`SELECT stock_actuel FROM articles WHERE id = ?`).get(ligne.article_id);
            if (!article) continue;
            ...
            db.prepare(`INSERT INTO mouvements_stock (article_id, type_mouvement, quantite, stock_avant, stock_apres, document_id, document_type, utilisateur_id) VALUES (?,?,?,?,?,?,?,?)`)
              .run(ligne.article_id, typeMvt, ligne.quantite || 1, stockAvant, stockApres, docId, type_document, req.user?.id);
            db.prepare(`UPDATE articles SET stock_actuel = ? WHERE id = ?`).run(stockApres, ligne.article_id);
          }
        });
        movBatch();
      }
    ...
    auditLog(db, req.user?.id, 'CREATION', type_document, docId, { numero });
  ...
  router.get('/', (req, res) => {
    ...
    res.json({ documents: db.prepare(sql).all(...params), total: total.total, page: parseInt(page) });
  });

  // GET /api/documents/:id
  router.get('/:id', (req, res) => {
    const doc = db.prepare(`SELECT d.*, c.raison_sociale as client_nom, c.ice as client_ice, c.adresse as client_adresse, c.ville as client_ville, c.telephone as client_tel, c.rc as client_rc, f.raison_sociale as fournisseur_nom FROM documents d LEFT JOIN clients c ON d.client_id = c.id LEFT JOIN fournisseurs f ON d.fournisseur_id = f.id WHERE d.id = ?`).get(req.params.id);
    if (!doc) return res.status(404).json({ error: 'Document introuvable' });
    doc.lignes = db.prepare(`SELECT dl.*, a.reference as art_reference, a.designation as art_designation FROM documents_lignes dl LEFT JOIN articles a ON dl.article_id = a.id WHERE dl.document_id = ? ORDER BY dl.ligne_numero`).all(req.params.id);
    res.json(doc);
  });
  ...
  router.put('/:id/statut', (req, res) => {
    ...
    db.prepare(`UPDATE documents SET statut = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(statut, req.params.id);

    if (doc.client_id && ['facture_client', 'avoir_client'].includes(doc.type_document)) {
      updateClientSolde(db, doc.client_id);
    }

    auditLog(db, req.user?.id, 'CHANGEMENT_STATUT', doc.type_document, req.params.id, { ancien: doc.statut, nouveau: statut });
    res.json({ success: true });
  });

  // POST /api/documents/:id/transfert - convert to next document type
  router.post('/:id/transfert', (req, res) => {
    const doc = db.prepare(`SELECT * FROM documents WHERE id = ?`).get(req.params.id);
    if (!doc) return res.status(404).json({ error: 'Document introuvable' });
    ...
    const lignes = db.prepare(`SELECT * FROM documents_lignes WHERE document_id = ?`).all(doc.id);
    if (!lignes.length) return res.status(400).json({ error: 'Aucune ligne à transférer' });

    // Créer le nouveau document
    const numero = generateDocumentNumber(db, nextType);
    const result = db.prepare(`INSERT INTO documents (type_document, numero, client_id, fournisseur_id, utilisateur_id, notes, date_document, statut) VALUES (?,?,?,?,?,?,CURRENT_DATE,'brouillon')`)
      .run(nextType, numero, doc.client_id, doc.fournisseur_id, req.user?.id, doc.notes);
    const newDocId = result.lastInsertRowid;

    // Copier les lignes
    const insertLigne = db.prepare(`INSERT INTO documents_lignes (document_id, article_id, ligne_numero, reference, designation, quantite, prix_unitaire_ht, remise_pourcent, taux_tva, montant_ht, montant_tva, montant_ttc) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`);
    let totalHT = 0, totalTVA = 0, totalTTC = 0;
    ...
    const insertBatch = db.transaction(() => {
      for (const l of lignes) {
        ...
        totalTVA += montantTVA;
        totalTTC += montantTTC;
        insertLigne.run(newDocId, l.article_id, l.ligne_numero, l.reference, l.designation, l.quantite, l.prix_unitaire_ht, l.remise_pourcent, l.taux_tva, montantHT, montantTVA, montantTTC);
      }
    });
    insertBatch();
    ...
    db.prepare(`UPDATE documents SET statut = 'valide', notes = COALESCE(notes,'') || '\nTransféré vers ' || (SELECT numero FROM documents WHERE id = ?) WHERE id = ?`)
      .run(newDocId, doc.id);

    auditLog(db, req.user?.id, 'TRANSFERT', doc.type_document, doc.id, { vers: nextType, new_id: newDocId, numero });
    res.status(201).json({ id: newDocId, numero, type: nextType });
  });

  // DELETE /api/documents/:id
  router.delete('/:id', (req, res) => {
    db.prepare(`UPDATE documents SET statut = 'annule', updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(req.params.id);
    res.json({ success: true });
  });

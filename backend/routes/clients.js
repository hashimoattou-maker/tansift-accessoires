...
module.exports = function(db) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      const last = await db.prepare(`SELECT code_client FROM clients ORDER BY id DESC LIMIT 1`).get();
      let nextNum = 1;
      if (last && last.code_client && last.code_client.startsWith('CLT-3421')) {
        nextNum = parseInt(last.code_client.slice(-4)) + 1;
      } else {
        const count = await db.prepare(`SELECT COUNT(*) as cnt FROM clients`).get();

...
module.exports = function(db) {
  ...
  router.get('/:id', async (req, res) => {
    try {
      const article = await db.prepare(`SELECT a.*, c.nom as categorie_nom FROM articles a LEFT JOIN categories c ON a.categorie_id = c.id WHERE a.id = ?`).get(req.params.id);
      if (!article) return res.status(404).json({ error: 'Article introuvable' });
      article.photos = await db.prepare(`SELECT * FROM articles_photos WHERE article_id = ? ORDER BY ordre`).all(req.params.id);
      article.references = await db.prepare(`SELECT * FROM articles_references WHERE article_id = ?`).all(req.params.id);
      article.compatibilites = await db.prepare(`SELECT * FROM articles_compatibilites WHERE article_id = ?`).all(req.params.id);
      if (article.est_moteur) {
        article.nomenclature = await db.prepare(`SELECT n.*, a.reference, a.designation FROM nomenclature_moteur n JOIN articles a ON n.composant_id = a.id WHERE n.moteur_id = ?`).all(req.params.id);
      }
      res.json(article);
    } catch (e) {
      res.status(500).json({ error: e.message });
  ...
  router.delete('/:id', async (req, res) => {
    try {
      const article = await db.prepare(`SELECT id FROM articles WHERE id = ?`).get(req.params.id);
      if (!article) return res.status(404).json({ error: 'Article introuvable' });
      await db.prepare(`UPDATE articles SET actif = 0 WHERE id = ?`).run(req.params.id);
      await auditLog(db, req.user?.id, 'SUPPRESSION', 'article', req.params.id, {});
      res.json({ success: true });
    } catch (e) {
  ...
  router.post('/:id/compatibilite', async (req, res) => {
    try {
      const { marque, modele, motorisation, annee_debut, annee_fin } = req.body;
      const result = await db.prepare(`INSERT INTO articles_compatibilites (article_id, marque, modele, motorisation, annee_debut, annee_fin) VALUES (?,?,?,?,?,?)`)
  ...
  router.post('/:id/reference', async (req, res) => {
    try {
      ...
      const result = await db.prepare(`INSERT INTO articles_references (article_id, type_reference, code, nom_fournisseur) VALUES (?,?,?,?)`)

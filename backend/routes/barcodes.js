...
module.exports = function(db) {
  ...
  router.get('/generate/:articleId', (req, res) => {
    ...
    if (!code) {
      ...
      db.prepare(`UPDATE articles SET code_barre = ? WHERE id = ?`).run(code, article.id);
    }

    const existing = db.prepare(`SELECT * FROM codes_barres WHERE article_id = ?`).get(article.id);
    if (!existing) {
      db.prepare(`INSERT INTO codes_barres (article_id, code, format) VALUES (?,?,?)`).run(article.id, code, 'Code128');
    }
  ...
  router.post('/scan', (req, res) => {
    ...
    const article = db.prepare(`SELECT a.*, c.nom as categorie_nom, t.taux as taux_tva_value FROM articles a LEFT JOIN categories c ON a.categorie_id = c.id LEFT JOIN taux_tva t ON a.tva_id = t.id WHERE a.code_barre = ? OR a.reference = ?`).get(code, code);
  ...
  router.post('/print-labels', (req, res) => {
    ...
    for (const id of article_ids) {
      ...
      if (a) {
        if (!a.code_barre) {
          ...
          db.prepare(`UPDATE articles SET code_barre = ? WHERE id = ?`).run(a.code_barre, a.id);
        }
        articles.push(a);
      }
    }
  ...
  router.get('/history', (req, res) => {
    const codes = db.prepare(`SELECT cb.*, a.reference, a.designation FROM codes_barres cb JOIN articles a ON cb.article_id = a.id ORDER BY cb.date_generation DESC LIMIT 50`).all();
    res.json(codes);
  });
